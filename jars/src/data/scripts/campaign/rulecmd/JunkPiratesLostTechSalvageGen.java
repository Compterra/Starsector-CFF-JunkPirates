...
//	}
	...
	public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params, final Map<String, MemoryAPI> memoryMap) {
		...
		SalvageEntityGenDataSpec spec = specId == null ? null : SalvageEntityGeneratorOld.getSalvageSpec(specId);
...
//		}
		...
		String factionId = "junk_pirates_losttech";
		FactionAPI entityFaction = entity.getFaction();
		if (entityFaction != null && entityFaction.getId() != null) {
			factionId = entityFaction.getId();
		}
...
//				}
				...
				CampaignFleetAPI defenders = FleetFactoryV3.createFleet(fParams);
                                ...
                                FactionAPI defenderFaction = defenders.getFaction();
                                if (defenderFaction != null) {
                                    defenderFaction.pickShipAndAddToFleet(ShipRoles.COMBAT_CAPITAL, ShipPickParams.all(), defenders);
                                    defenderFaction.pickShipAndAddToFleet(ShipRoles.COMBAT_CAPITAL, ShipPickParams.all(), defenders);
                                }
