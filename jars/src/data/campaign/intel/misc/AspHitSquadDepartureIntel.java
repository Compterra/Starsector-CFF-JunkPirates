...
//		data = (EconomyFleetAssignmentAI.EconomyRouteData) route.getCustom();
                aspFaction = Global.getSector() != null ? Global.getSector().getFaction("syndicate_asp") : null;
	...
	protected void addBulletPoints(TooltipMakerAPI info, IntelInfoPlugin.ListInfoMode mode) {
		...
		if (mode != IntelInfoPlugin.ListInfoMode.IN_DESC && aspFaction != null) {
...
//			}
	...
	protected String getWhat() {
                ...
                String what = Global.getSector() != null && Global.getSector().getPlayerPerson() != null ? Global.getSector().getPlayerPerson().getNameString() : "the target";
...
//		label.setHighlightColors(data.from.getFaction().getBaseUIColor(), data.from.getFaction().getBaseUIColor(), data.to.getFaction().getBaseUIColor());
		...
		LabelAPI label = info.addPara("You are getting a lot of comms traffic from " + data.from.getName() +
                ...
                LabelAPI label2 = info.addPara("It appears that " + aspPrefixArticle + " " +
				 aspPrefix + " " + fleetType +
                                " was seen in orbit, seeking not much else but your whereabouts.",
				 opad, tc);

		label.setHighlight(data.from.getName(), aspPrefix);
		label.setHighlightColors(data.from.getFaction().getBaseUIColor(), aspColor);
...
//                }
		...
		String what = getWhat();
                ...
                if ("tiny".equals(fleetSizeDescriptor) || "small".equals(fleetSizeDescriptor)) {
		...
		label3.setHighlightColors(aspColor);
	...
	public Set<String> getIntelTags(SectorMapAPI map) {
		...
		if (aspFaction != null) tags.add(aspFaction.getId());
